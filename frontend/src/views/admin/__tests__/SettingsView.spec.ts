...
vi.mock("vue-i18n", async () => {
  ...
  const translations: Record<string, string> = {
    ...
    "admin.settings.platformQuota.daily": "日限额 (RMB)",
    "admin.settings.platformQuota.weekly": "周限额 (RMB)",
    "admin.settings.platformQuota.monthly": "月限额 (RMB, 30天滚动)",
...
describe("admin SettingsView platform quota matrix", () => {
  ...
  it("从 baseSettings 加载默认平台配额数据并在 Users tab 渲染全平台行", async () => {
    const wrapper = mountView();
    await flushPromises();
    await openUsersTab(wrapper);
  ...
  it("保存时 updateSettings payload 应包含嵌套 default_platform_quotas 对象（含全平台）", async () => {
    ...
    const platforms = ["anthropic", "openai", "gemini", "antigravity", "deepseek"];
  ...
  it("加载后 form.default_platform_quotas 含全平台，从嵌套 JSON 正确读取数值", async () => {
    getSettings.mockResolvedValueOnce({
      ...baseSettingsResponse,
      default_platform_quotas: {
        anthropic: { daily: 5, weekly: null, monthly: null },
        openai:    { daily: null, weekly: 12.5, monthly: null },
        // gemini / antigravity / deepseek 缺失 → 应被归一化为全 null
      },
    });
