...
const form = reactive({ email: '', password: '', username: '', notes: '', role: 'user' as AdminUser['role'], concurrency: 1, rpm_limit: 0, customAttributes: {} as UserAttributeValuesMap })
...
const handleUpdateUser = async () => {
  ...
  try {
    ...
    if (authStore.isSuperAdmin) data.role = form.role
